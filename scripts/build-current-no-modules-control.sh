...
kernel="$base/out/kernel-gts9uwifi-v032"
bundle="$base/out/gts9uwifi-mainline-v0.32"
overlay="$base/out/rootfs-overlay-v0.32"
artifact=${ARTIFACT:-"$project/artifacts/postmarketos-edge-xfce-mainline-v0.32-current-kernel-goodix-no-modules-sm-x910-twrp.zip"}

KERNEL_OUT_DIR="$kernel" bash "$project/scripts/build-mainline-kernel.sh"

case "$overlay" in
	"$base"/out/rootfs-overlay-v0.32) rm -rf -- "$overlay" ;;
...
python3 "$project/scripts/make-twrp-zip.py" "$bundle" "$artifact" \
	...
	--label 'postmarketOS mainline v0.32 for SM-X910 (current kernel, complete Goodix fix, no modules)'
