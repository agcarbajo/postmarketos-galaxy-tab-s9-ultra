...
main_apk="$package_dir/mutter-999950.2-r5.apk"

if [ "${FORCE_MUTTER_BUILD:-0}" != 1 ] &&
	test -f "$main_apk" &&
	test -f "$package_dir/mutter-lang-999950.2-r5.apk" &&
	test -f "$package_dir/mutter-schemas-999950.2-r5.apk" &&
	test -f "$package_dir/mutter-udev-999950.2-r5.apk"; then
	echo "Reusing already-built mutter 999950.2-r5 packages"
	exit 0
fi
